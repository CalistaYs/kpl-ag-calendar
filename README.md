...
- 本次解析到的比赛数量是否比上一次 `calendar.ics` 里**同一个赛季**的数量少了一半以上（可能意味着
  数据源出了问题）——这里特意只跟同一赛季比，不能跟日历里累计的全部历史比赛数比，否则赛季一多、
  历史事件越攒越多，新赛季刚开赛、比赛数量还很少时就会被永远误判成异常

**任何一项检查失败，脚本都会保留现有 `calendar.ics` 不动，打印详细错误后以非零状态退出**——
这样 GitHub Actions 那次运行会显示失败（方便在 Actions 日志里定位原因），而不是把有问题的数据
悄悄提交上去覆盖旧日历。

校验通过后，脚本会先把新内容（跟历史比赛合并后的完整结果，见下面"UID、跨赛季历史保留"一节）
写到 `calendar.new.ics`，确认合并后的事件数正好等于"其它赛季历史比赛数 + 本次抓到的比赛数"后，
...
## UID、跨赛季历史保留与比赛延期/改期/取消/重赛

每场比赛的 `UID` 直接基于官方 `scheduleid`（如 `KPL2026S2M3W3D3`）生成，这是腾讯赛程系统给这场
比赛分配的稳定标识，不会随开赛时间、地点、比分变化，而且天然带着赛季信息（scheduleid 本身以
`KPL{年}S{1/2}` 开头）。

`update_calendar.py` 每次运行只会向官方接口请求"当前赛季"的比赛，但写回 `calendar.ics` 时会跟
文件里已有的内容合并（[`ics_generator.merge_calendars()`](ics_generator.py)），而不是整份覆盖：

- **不属于当前赛季的历史比赛**（UID 前缀是其它赛季）——也就是这次请求根本没有触及、已经切换过去的
  赛季——原样保留，不会因为跨赛季就从日历里消失，未被触及的历史事件也保留原有的 `DTSTAMP`。
- **属于当前赛季的比赛**，以这次抓取的完整结果为准：官方修改开赛时间/地点、更新比分，重新抓取后
  会更新同一个日历事件（`DTSTART`/`DTEND`/`LOCATION`/`DESCRIPTION`），不会产生重复事件；如果一场
  当季比赛从官方接口的返回结果里彻底消失（比如被取消），下次刷新后它也会跟着从这一季里移除——
  因为每次都是拉取"该赛季的完整赛程"，不是增量更新，所以能安全地这样处理，不会误删其它赛季的
  历史记录。
