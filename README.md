...
赛程表按 Wikipedia 页面真实的 `<tr>`/`<td>` 行列结构解析：每一行要么是跨列的日期行，要么是一场比赛
（`[队伍, 队伍]` 表示未开赛，或 `[队伍, 比分, 比分, 队伍]` 表示已完赛），不符合这两种形状的行会被跳过。
早期版本是把整页拍平成一条线性 token 序列再向后扫描，会跨越表格行边界，把积分榜、名单等其他表格里的
数字和队名误当成比赛数据，导致出现日期/比分错误、同一场比赛重复出现的问题；现在按行解析后不会再有这类问题。
